AFRAME.registerComponent('event-manager', {
	init: function () {
		...
		this.exitButtonEl = document.querySelector('#exitButton');
		this.buttonToGeometry = { 'boxButton': this.boxGeometryEl, 'sphereButton': this.sphereGeometryEl, 'torusButton': this.torusGeometryEl };
		...
		this.exitButtonEl.addEventListener('click', this.onClick);
	...
	onClick: function (evt) {
		...
		if (targetEl === this.boxButtonEl || targetEl === this.sphereButtonEl || targetEl === this.torusButtonEl) {
		...
		if (targetEl === this.exitButtonEl) {
			this.exitButtonEl.removeState('pressed');
			this.exitButtonEl.sceneEl.renderer.xr.getSession().end();
		}
		else {
